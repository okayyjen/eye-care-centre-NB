export default function ContactPage(){
    return(
        <div>
            <div>
                <div className="flex justify-center flex-wrap mt-[50px]">
                    <div className="w-[600px] h-auto bg-white p-5">
                        <h2 className="text-[25px]">Contact Us</h2>
                        <section className="select-none my-[-5px] mt-[-5px] mb-[10px]" >______</section>
                        
                            <div className="mb-[10px]">
                                <h1 className="mb-[10px]" ><strong>NB Eye Care Centre</strong></h1>

                                <div className="mb-[10px]">
                                    <p className = "mb-[10px]">5085 Principale-Est Ave</p>
                                    <p className = "mb-[10px]">Dieppe, NB</p>
                                    <p className = "mb-[10px]">Tel: (506) 830-1773</p>
                                </div>

                                <div className="flex flex-col mb-[10px]">
                                    <div className="flex justify-between py-[2px]">
                                        <span className="flex-1 text-left"><strong>Monday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]">
                                        <span className="flex-1 text-left"><strong>Tuesday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]-row">
                                        <span className="flex-1 text-left"><strong>Wednesday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]-row">
                                        <span className="flex-1 text-left"><strong>Thursday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]-row">
                                        <span className="flex-1 text-left"><strong>Friday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]-row">
                                        <span className="flex-1 text-left"><strong>Saturday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                    <div className="flex justify-between py-[2px]-row">
                                        <span className="flex-1 text-left"><strong>Sunday:</strong></span>
                                        <span className="flex-1 text-left">8 am – 5 pm</span>
                                    </div>
                                </div>

                                <div className="mb-[10px]">
                                    <div><a href="mailto:example@example.com">example@example.com</a></div>
                                </div>
                            </div>

                    </div>
                    <div className = "h-[600px] md:h-[432px]">
                        <iframe 
                        src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d5533.1938555553215!2d-64.70164862244005!3d46.09901127109149!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x4ca0bc71b73b6ff5%3A0xa1ec43d3eb6741d2!2sEye%20Care%20Centre%20NB%20-%20Dr.%20Houfar%20Sekhavat%20Dr.%20Isabelle%20Savoie%20Dr.%20Julien%20Saad%20Dr.%20Jose%20Ponce%20Dr.%20Hila%20Zommer!5e0!3m2!1sen!2sca!4v1718230408817!5m2!1sen!2sca" 
                        width="600" 
                        height="100%" 
                        style={{border:0}} 
                        allowfullscreen="" 
                        loading="lazy" 
                        referrerpolicy="no-referrer-when-downgrade">
                        </iframe> 
                    </div>
                </div> 
            </div>
        </div>
    )
}